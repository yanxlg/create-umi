...
const BasicGenerator = require('create-umi/lib/BasicGenerator');
...
class AntDesignProGenerator extends BasicGenerator {
  ...
  async writing() {
    ...
    if (!isTypeScript) {
      ...
      fs.copySync(path.resolve(__dirname, 'copy/javascript'), path.resolve(projectPath, ''));

    }else{
      // typescript 定制化
      fs.copySync(path.resolve(__dirname, 'copy/typescript'), path.resolve(projectPath, ''));
      // eslint 规则
      // config 文件修改
      let config = fs.readFileSync(path.resolve(projectPath, 'config/config.ts'),'utf-8');
      if(/getLocalIdent/.test(config)){
        config= config.replace("context.resourcePath.includes('node_modules')","context.resourcePath.includes('node_modules') || /less[\\/|\\\\]\\S+.less$|icons[\\/|\\\\]iconfont.css$/.test(context.resourcePath)");
      }
      config = config.replace("antd-pro",projectName);
      fs.writeFileSync(path.resolve(projectPath, 'config/config.ts'),config);

      const defaultSetting = fs.readFileSync(path.resolve(projectPath, 'config/defaultSettings.ts'),'utf-8').replace("Ant Design Pro",this.opts.args.title||"<title>");
      fs.writeFileSync(path.resolve(projectPath, 'config/defaultSettings.ts'),defaultSetting);
    }
    ...
    // extra clean up...
    log('Clean up...');
    fs.removeSync(path.resolve(projectPath, ".github"));
