...
const umi_generators = fs
  .readdirSync(`${__dirname}/../node_modules/create-umi/lib/generators`)
...
const run = async config => {
  ...
  if(fs.existsSync(config.name)&&fs.readdirSync(config.name).length>0){
    // 文件夹已存在并存在文件
    console.error(chalk.red(`> fatal`), "destination path 'a1' already exists and is not an empty directory");
    const clean = await inquirer.prompt([
      {
        name: 'type',
        message: 'Do you want to empty the directory？',
        type: 'list',
        choices: [{
          name:`YES - ${chalk.red('will empty the directory with drop original files!')}`,
          value:"YES",
          short:"YES"
        },{
          name:`NO - ${chalk.red('will exit with 1!')}`,
          value:"NO",
          short:"NO"
        }],
      },
