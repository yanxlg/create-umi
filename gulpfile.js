...
gulp.task(
    'publish',
    gulp.series(done => {
        console.log(chalk.greenBright('publishing'));
        const { version } = pkg;
        ...
        console.log(chalk.greenBright('published'));
        if (!ret) {
            ret = undefined;
        }
        done(ret);
...
gulp.task(
    ...
    gulp.series('publish', done => {
        ...
        console.log(chalk.greenBright("update version"));
        shelljs.exec(`git tag ${version}`);
        shelljs.exec(`git push origin ${version}:${version}`);
        shelljs.exec('git push origin master:master');
        console.log(chalk.greenBright('tagged'));
        done();
    })
);